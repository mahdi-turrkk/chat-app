...
<!--    navigation buttons-->
<!--    previous button-->
    <div :class="{'translate-x-10' : !showNavigation}" class="absolute top-0 bottom-0 right-0 flex items-center transition-all duration-400" v-if="!hideNavigators">
      <slot name="previousButton" :onclick="handlePreviousButton">
        <button class="bg-background-1 rounded-full shadow-lg mr-3" @click="handlePreviousButton">
          <i class="pi pi-chevron-right txt-2xl text-black"></i>
        </button>
      </slot>
    </div>
<!--    next button-->
    <div :class="{'-translate-x-10' : !showNavigation}" class="absolute top-0 bottom-0 left-0 flex items-center transition-all duration-400" v-if="!hideNavigators">
      <slot name="nextButton" :onclick="handleNextButton">
        <button class="bg-background-1 rounded-full drop-shadow-lg ml-3" @click="handleNextButton">
          <i class="pi pi-chevron-left txt-2xl text-black"></i>
        </button>
      </slot>
    </div>
  </div>
        ...
        <div v-for="i in data.length">
          <slot name="slideIndicatorItem" :onclick="handleIndicatorClick" :item="data[i-1]" :index="i">
            <div class="h-2 rounded-full transition-all duration-300"
                 :class="onboarding === i-1 ? 'bg-primary-default w-3' : 'bg-primary-disabled w-2 hover:bg-primary-default'"
